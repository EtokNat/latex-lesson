...
/* ============================================================
   Progressive Aligned Equation Component
   ============================================================ */
const ProgressiveAlignedEquation: React.FC<ProgressiveAlignedEquationProps> = ({
  ...
  console.log('[ProgressiveAlignedEquation] Mount/Update - equationString:', equationString?.substring(0, 60), 'revealCount:', revealCount, 'displayMode:', displayMode);

  const tokens = useMemo(() => {
    console.log('[ProgressiveAlignedEquation] Tokenizing equation');
    let cleanEq = equationString.trim();
    ...
    const lines = smartSplitLines(cleanEq);
    console.log('[ProgressiveAlignedEquation] Split into', lines.length, 'lines');
    return lines.map(parseEquation);
  }, [equationString]);

  const lineTotals = useMemo(() => {
    const totals = tokens.map(t => t.reduce((s, n) => s + totalReveal(n), 0));
    console.log('[ProgressiveAlignedEquation] Line totals:', totals);
    return totals;
  }, [tokens]);

  const counts = useMemo(() => {
    let remaining = revealCount;
    const result = lineTotals.map(total => {
      const used = Math.min(remaining, total);
      remaining -= used;
      return used;
    });
    console.log('[ProgressiveAlignedEquation] Distributed counts:', result, 'remaining:', remaining);
    return result;
  }, [revealCount, lineTotals]);
  ...
  const katexHtml = useMemo(() => {
    ...
    console.log('[ProgressiveAlignedEquation] Frontier line index:', frontierLineIndex);

    const isCustomRootEnv = /^\s*\\begin\{[a-zA-Z*]+\}/.test(equationString.trim()) && !equationString.trim().startsWith("\\begin{aligned}");
    ...
    try {
      ...
      console.log('[ProgressiveAlignedEquation] KaTeX rendered successfully, HTML length:', html.length);
      return html;
    } catch (e) {
      console.error('[ProgressiveAlignedEquation] KaTeX rendering error:', e);
      return `<div style="color:red;font-family:monospace;">Render Error: Check Console</div>`;
    }
  }, [tokens, counts, equationString, displayMode]);
